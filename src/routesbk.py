...
def contactlist():
    ...
    sql = text("SELECT P.firstname,P.lastname,P.message,P.user_id,F.file_name FROM contact C JOIN profile P ON C.contact_id = P.user_id LEFT JOIN photos F ON F.user_id=C.contact_id  WHERE C.user_id=:id")
    result = db.session.execute(sql, {"id":id})   
...
def upload_file():
    ...
    sql = text("INSERT INTO photos (user_id,file_name) VALUES (:id,:filename) ON CONFLICT (user_id) DO UPDATE SET file_name = EXCLUDED.file_name")
